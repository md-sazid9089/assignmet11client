  ...
  const { data: transactionData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/transactions/admin/all`,
        { 
          headers: { 
            'x-user-id': userId
          } 
      ...
      >
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">
            All Platform <span className="text-[#b35a44]">Transactions</span>
          </h1>
          <p className="text-slate-400">
            Monitor all payment transactions across the entire platform
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {[
            ...
            {
              title: 'Total Revenue',
              value: `৳${transactions.reduce((sum, t) => sum + (t.amount || 0), 0).toLocaleString()}`,
              icon: <DollarSign className="w-6 h-6" />,
              color: 'from-[#b35a44] to-orange-400'
            },
            {
              title: 'Unique Users',
              value: new Set(transactions.map(t => t.userEmail)).size,
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          {/* Status Filters */}
          {[
            { key: 'all', label: 'All Transactions', icon: <Filter className="w-4 h-4" /> },
          ...
          <select
            ...
            className="flex items-center space-x-2 px-4 py-2 rounded-xl border bg-slate-800/50 text-slate-400 border-slate-700 hover:bg-slate-700/50 hover:text-white focus:outline-none"
          >
          ...
          <div className="relative bg-slate-900/70 backdrop-blur-xl border border-slate-700/50 rounded-3xl overflow-hidden">
            {sortedTransactions.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-400">
                  {filter === 'all' ? 'No transactions available yet.' : `No ${filter} transactions found.`}
                </p>
              </div>
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-700/50">
                      ...
                      <th className="text-left p-6 text-slate-300 font-semibold">User</th>
                      <th className="text-left p-6 text-slate-300 font-semibold">Ticket</th>
                      <th className="text-left p-6 text-slate-300 font-semibold">Amount</th>
                  ...
                  <tbody>
                    {sortedTransactions.map((transaction, index) => (
                      ...
                      >
                        ...
                        <td className="p-6">
                          <div className="flex items-center space-x-2">
                            <DollarSign className="w-4 h-4 text-[#b35a44]" />
